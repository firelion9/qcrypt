...
import com.firelion.crystals.struct.*
 ...
 * vector of polynomials [KyberPublicKey.key]
 * and the public [KyberPublicKey.seed] used to generate the matrix A.
 */
internal fun packPublicKey(publicKey: KyberPublicKey, out: ByteArray) {
    publicKey.getEncoded(out)
}
 ...
 */
internal fun unpackPublicKey(bytes: ByteArray, bytesOffset: Int, out: KyberPublicKey, params: KyberParams) {
    PolynomialVector.fromBytes(bytes, bytesOffset, out.key)
    (0 until KYBER_SYMMETRIC_BYTES).forEach { i ->
        out.seed[i] = bytes[bytesOffset + params.KYBER_POLYNOMIAL_VECTOR_BYTES + i]
    }
}
 ...
 */
internal fun packPrivateKey(privateKey: KyberPrivateKey, out: ByteArray) {
    privateKey.getEncoded(out)
}

/**
 * De-serialize the secret key; inverse of [packPrivateKey]
 */
internal fun unpackPrivateKey(bytes: ByteArray, out: KyberPrivateKey) {
    PolynomialVector.fromBytes(bytes, 0, out.key)
}
...
internal fun indcpaKeyPair(publicKey: ByteArray, privateKey: ByteArray, params: KyberParams) {
    ...
    packPrivateKey(KyberPrivateKey(privateKeyVec), privateKey)
    packPublicKey(KyberPublicKey(publicKeyVec, buf, params.KYBER_POLYNOMIAL_VECTOR_BYTES), publicKey)
}
...
internal fun indcpaEncrypt(
    ...
    unpackPublicKey(publicKey, publicKeyOffset, key, params)
    Polynomial.fromMsg(message, k)
    genAT(at, key.seed, params)
    ...
    key.key.baseMulAccumulatedMontgomery(sp, v)
    b.inverseNttToMont()
    v.inverseNttToMont()
...
internal fun indcpaDecrypt(
    ...
    val key = KyberPrivateKey(params)
    ...
    text.b.ntt()
    key.key.baseMulAccumulatedMontgomery(text.b, mp)
    mp.inverseNttToMont()
